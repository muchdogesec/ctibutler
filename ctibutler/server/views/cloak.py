...
from ctibutler.server.arango_helpers import ArangoDBHelper, CLOAK_TYPES, CLOAK_FORMS, CTI_SORT_FIELDS
...
@extend_schema_view(
    create=extend_schema(
        ...
        summary="Download CLOAK objects",
        description=textwrap.dedent(
            """
            Use this data to update CLOAK records. CLOAK stands for Concealment Layers for Online Anonymity and Knowledge.

            The following key/values are accepted in the body of the request:

            * `version` (required): the version of CLOAK you want to download in the format `YY.MM.DD` or `YY_MM_DD`, e.g. `26.06.19` or `26_06_19`. You can see all versions installed and available to download on the version endpoints.
    ...
    list_objects=extend_schema(
        summary='Search and filter CLOAK objects',
        description=textwrap.dedent(
            """
            Search and filter CLOAK objects.
            """
        ),
        filters=True,
    ...
    retrieve_objects=extend_schema(
        summary='Get a CLOAK object',
        description=textwrap.dedent(
            """
            Get a CLOAK object by its ID (e.g. `ta3-te9` `ta4-te4-st348`) OR the STIX ID e.g. `attack-pattern--cbc8801b-17b8-5ba8-9b31-034c7e9e46f8`.

            If you do not know the ID of the object you can use the GET CLOAK Objects endpoint to find it.
            """
        ),
        filters=False,
        responses={200: serializers.StixObjectsSerializer(many=True), 400: DEFAULT_400_ERROR},
    ...
    object_versions=extend_schema(
        summary="See all versions of the CLOAK object",
        description=textwrap.dedent(
            """
            This endpoint will show the STIX versions of the object (`modified` property) and what CLOAK versions it appears in.
    ...
    retrieve_object_relationships=extend_schema(
        summary='Get the Relationships linked to the CLOAK Object',
        description=textwrap.dedent(
            """
            This endpoint will return all the STIX relationship objects where the CLOAK object is found as a `source_ref` or a `target_ref`.

            If you want to see an overview of how CLOAK objects are linked, see the relationship diagram.
            """
        ),
    ...
    bundle=extend_schema(
        summary='Get all objects linked to the CLOAK Object',
        description=textwrap.dedent(
            """
    ...
    truncate=extend_schema(
        summary=f"Wipe the collections holding CLOAK objects",
        description=textwrap.dedent(
            f"""
            Wipe the ArangoDB Collections `cloak_vertex_collection` and `cloak_edge_collection` holding CLOAK objects.

            **WARNING**: This will delete all objects in these collections, which will mean all CLOAK versions stored will be removed.
            """
        ),
    ),
)  
class CloakView(TruncateView, viewsets.ViewSet):
    openapi_tags = ["CLOAK"]
    lookup_url_kwarg = 'cloak_id'
    ...
    class filterset_class(FilterSet):
        ...
        types = ChoiceCSVFilter(choices=[(f,f) for f in CLOAK_TYPES], help_text='Filter the results by STIX Object type.')
        cloak_version = CharFilter(help_text="By default only the latest CLOAK version objects will be returned. You can enter a specific CLOAK version here. e.g. `26.06.19` or `26_06_19`. You can get a full list of versions on the GET CLOAK versions endpoint.")
        cloak_type = ChoiceCSVFilter(choices=[(f,f) for f in CLOAK_FORMS], help_text='Filter the results by CLOAK Object type.')
        SORT_FIELDS = CTI_SORT_FIELDS+['cloak_id_ascending', 'cloak_id_descending']
    ...
    def create(self, request, *args, **kwargs):
        ...
        job = new_task(data, models.JobType.CLOAK_UPDATE)
    ...
    def list_objects(self, request, *args, **kwargs):
        return ArangoDBHelper(self.arango_collection, request).get_weakness_or_capec_objects(types=CLOAK_TYPES, lookup_kwarg=self.lookup_url_kwarg, forms=CLOAK_FORMS)
    
    @extend_schema(
            parameters=[
                OpenApiParameter('cloak_version', description="By default only the latest CLOAK version objects will be returned. You can enter a specific CLOAK version here. e.g. `26.06.19` or `26_06_19`. You can get a full list of versions on the GET CLOAK versions endpoint.")
            ],
    )
    @decorators.action(methods=['GET'], url_path="objects/<str:cloak_id>", detail=False)
    def retrieve_objects(self, request, *args, cloak_id=None, **kwargs):
        return ArangoDBHelper(self.arango_collection, request).get_object_by_external_id(cloak_id, self.lookup_url_kwarg.replace('_id', '_version'))
    ...
    @decorators.action(methods=['GET'], url_path="objects/<str:cloak_id>/relationships", detail=False)
    def retrieve_object_relationships(self, request, *args, cloak_id=None, **kwargs):
        return ArangoDBHelper(self.arango_collection, request).get_object_by_external_id(cloak_id, self.lookup_url_kwarg.replace('_id', '_version'), relationship_mode=True)    
    ...
    @decorators.action(methods=['GET'], url_path="objects/<str:cloak_id>/bundle", detail=False)
    def bundle(self, request, *args, cloak_id=None, **kwargs):
        return ArangoDBHelper(self.arango_collection, request).get_object_by_external_id(cloak_id, self.lookup_url_kwarg.replace('_id', '_version'), bundle=True)
        
    @extend_schema(
        summary="See installed CLOAK versions",
        description=textwrap.dedent(
            """
            It is possible to install multiple versions of CLOAK using the POST CLOAK endpoint. By default, all endpoints will only return the latest version of CLOAK objects (which generally suits most use-cases).

            This endpoint allows you to see all installed versions of CLOAK available to use, and which version is the latest (the default version for the objects returned).
            """
            ),
        )
    ...
    @decorators.action(methods=['GET'], url_path="objects/<str:cloak_id>/versions", detail=False, serializer_class=serializers.MitreObjectVersions(many=True), pagination_class=None)
    def object_versions(self, request, *args, cloak_id=None, **kwargs):
        return ArangoDBHelper(self.arango_collection, request).get_mitre_modified_versions(cloak_id, source_name='cloak')
