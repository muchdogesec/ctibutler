"""D3FEND View for handling D3FEND (NSA's Defensive Countermeasures Knowledge Base) objects."""
...
from ctibutler.server.arango_helpers import CTI_SORT_FIELDS, ArangoDBHelper
...
@extend_schema_view(
    create=extend_schema(
                ...
                examples=[
                    OpenApiExample(
                        ...
                        value={
                            ...
                            "parameters": {"version": "0_12_0_beta"},
                        },
                    )
                ],
            ),
        ...
        summary="Download D3FEND objects",
        description=textwrap.dedent(
            """
            Use this data to update D3FEND records. [More information about D3FEND here](https://d3fend.mitre.org/).
    ...
    list_objects=extend_schema(
        summary="Search and filter D3FEND objects",
        description=textwrap.dedent(
            """
            Search and filter D3FEND objects including offensive techniques, defensive techniques, and artifacts.
            """
        ),
        filters=True,
        responses={
    ...
    retrieve_object_relationships=extend_schema(
        summary="Get the Relationships linked to the D3FEND Object",
        description=textwrap.dedent(
            """
            This endpoint will return all the STIX relationship objects where the D3FEND object is found as a `source_ref` or a `target_ref`.
            """
        ),
        responses={
    ...
    bundle=extend_schema(
        summary="Get all objects linked to the D3FEND Object",
        description=textwrap.dedent(
            """
    ...
    object_versions=extend_schema(
        summary="See all versions of the D3FEND object",
        description=textwrap.dedent(
            """
            This endpoint will show the STIX versions of the object (`modified` property) and what D3FEND versions it appears in.
    ...
    truncate=extend_schema(
        summary=f"Wipe the collections holding D3FEND objects",
        description=textwrap.dedent(
            f"""
            Wipe the ArangoDB Collections `d3fend_vertex_collection` and `d3fend_edge_collection` holding D3FEND objects.

            **WARNING**: This will delete all objects in these collections, which will mean all D3FEND versions stored will be removed.
            """
        ),
    ),
)
class D3fendView(TruncateView, viewsets.ViewSet):
    openapi_tags = ["D3FEND"]
    lookup_url_kwarg = "d3fend_id"
    collection_to_truncate = "d3fend"
    bucket_name = "d3fend"
    openapi_path_params = [
        OpenApiParameter(
            "stix_id",
    ...
    class filterset_class(FilterSet):
        ...
        text = CharFilter(
            help_text="Filter the results by the `name` and `description` property of the object. Search is a wildcard, so `firewall` will return all descriptions that contain the string `firewall`."
        )
        name = CharFilter(
            help_text="Filter results by `name`. Is wildcard so `fire` will match `firewall`, `firewire`, etc."
        )
        types = ChoiceCSVFilter(
            choices=[(f, f) for f in D3FEND_TYPES],
        ...
        d3fend_type = ChoiceCSVFilter(
            ...
            help_text="Filter the results by D3FEND Object type.",
        )
        SORT_FIELDS = CTI_SORT_FIELDS + ["d3fend_id_ascending", "d3fend_id_descending"]
        sort = ChoiceFilter(
    ...
    def create(self, request, *args, **kwargs):
        ...
        job = new_task(data, models.JobType.D3FEND_UPDATE)
    ...
    def list_objects(self, request, *args, **kwargs):
        return ArangoDBHelper(
            "d3fend_vertex_collection", request
        ).get_weakness_or_capec_objects(
            types=D3FEND_TYPES, lookup_kwarg=self.lookup_url_kwarg, forms=D3FEND_FORMS
        )
    ...
    @decorators.action(
        methods=["GET"], url_path="objects/<str:d3fend_id>", detail=False
    )
    def retrieve_objects(self, request, *args, d3fend_id=None, **kwargs):
        return ArangoDBHelper(
            "d3fend_vertex_collection", request
        ).get_object_by_external_id(
            d3fend_id, self.lookup_url_kwarg.replace("_id", "_version")
        )
    ...
    @decorators.action(
        methods=["GET"], url_path="objects/<str:d3fend_id>/relationships", detail=False
    )
    def retrieve_object_relationships(self, request, *args, d3fend_id=None, **kwargs):
        return ArangoDBHelper(
            "d3fend_vertex_collection", request
        ).get_object_by_external_id(
            d3fend_id,
    ...
    @decorators.action(
        methods=["GET"], url_path="objects/<str:d3fend_id>/bundle", detail=False
    )
    def bundle(self, request, *args, d3fend_id=None, **kwargs):
        return ArangoDBHelper(
            "d3fend_vertex_collection", request
        ).get_object_by_external_id(
            d3fend_id, self.lookup_url_kwarg.replace("_id", "_version"), bundle=True
        )

    @extend_schema(
        summary="See installed D3FEND versions",
        description=textwrap.dedent(
            """
            It is possible to install multiple versions of D3FEND using the POST D3FEND endpoint. By default, all endpoints will only return the latest version of D3FEND objects (which generally suits most use-cases).

            This endpoint allows you to see all installed versions of D3FEND available to use, and which version is the latest (the default version for the objects returned).
            """
        ),
    )
    @decorators.action(
    ...
    def versions(self, request, *args, **kwargs):
        return ArangoDBHelper("d3fend_vertex_collection", request).get_mitre_versions()

    @extend_schema(filters=False)
    @decorators.action(
        methods=["GET"],
        url_path="objects/<str:d3fend_id>/versions",
        detail=False,
        serializer_class=serializers.MitreObjectVersions(many=True),
        pagination_class=None,
    )
    def object_versions(self, request, *args, d3fend_id=None, **kwargs):
        return ArangoDBHelper(
            f"d3fend_vertex_collection", request
        ).get_mitre_modified_versions(d3fend_id, source_name="d3fend")
